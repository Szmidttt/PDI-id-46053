...
\begin{itemize}
    \item \textbf{Analiza zrzutu pamięci}
    \begin{itemize}
        \item {DumpHasStream()}
        \item {DumpHasInstructionPointerMemory()}
        \item {DumpCheckMemory()}
    \end{itemize}
\item \textbf{Wywoływanie awarii procesu}
\begin{itemize}
        \item {Crash()}
        \item {EnablePHC()}
        \item {GetWin64CFITestFnAddrOffset()}
        \item {TryOverrideExceptionHandler()}
        \item {SaveAppMemory()}
    \end{itemize}
\end{itemize}
Dodatkowo weryfikowano brak regresji, rozumiany jako:
\begin{itemize}
    \item {zachowanie niezmienionego ABI biblioteki,}
    \item {identyczne zachowanie funkcji eksportowanych,}
    \item {przejście wszystkich testów XPCShell bez modyfikacji po stronie testów.}
\end{itemize}
W ramach analizy porównano również czas kompilacji oraz rozmiar wygenerowanej biblioteki przed i po migracji.
Metody pomiarowe:
\begin{itemize}
    \item {czas kompilacji, raportowany przez mach,}   
    \item {rozmiar pliku przez PowerShell:} \texttt{(Get-Item "testcrasher.dll").Length}
\end{itemize}
...
W trakcie migracji zidentyfikowano następujące problemy i utrudnienia:
\begin{itemize}
