...
\subsection{Reimplemetacja części odpowiedzialnej za analizę w języku Rust}
Zgodnie z przyjętą strategią iteracyjnej migracji, opisaną w rozdziale 3.3.1, proces reimplementacji rozpoczął się od modułu odpowiedzialnego za analizę zrzutów pamięci (dumputils.cpp). Kluczowym celem tego etapu było zastąpienie zewnętrznej zależności C++ - biblioteki Google Breakpad jej nowoczesnym i bezpiecznym odpowiednikiem w ekosystemie Rust, czyli rust-minidump. Poniższa analiza przedstawia kluczowe zmiany w kodzie, pogrupowane w kategorie tematyczne, które bezpośrednio odnoszą się do celów pracy - poprawy bezpieczeństwa i nowoczesności kodu.
\subsection{Zastąpienie zależności i nowy model obsługi błędów}
\input{code_frags/style.tex}
...
Jako że środowisko budowania zostało już wcześniej przygotowane do obsługi kodu Rust, prace mogły skupić się wyłącznie na implementacji.
\subsection{Reimplemetacja części odpowiedzialnej za wywoływanie awarii procesu w języku Rust}
\subsection{Architektura biblioteki testcrasher po migracji części odpowiedzialnej za wywoływanie awarii}

\section{Architektura biblioteki testcrasher po migracji}
\begin{figure}[H]
\centering
\includegraphics[width=0.8\linewidth]{figures/testcrasher2.png}
\caption{Architektura biblioteki testcrasher po 2 etapie - opracowanie własne}
