...
\begin{itemize}
    \item C2Rust framework umożliwiający translację kodu C (i częściowo C++) do Rust, wykorzystujący Clang do parsowania kodu źródłowego. Narzędzie generuje niskopoziomowy kod, który wymaga późniejszej refaktoryzacji (np. wprowadzenia bezpiecznych abstrakcji). \cite{c2rustmanual}
    \item Bindgen narzędzie rozwijane przez Mozilla, automatycznie generujące powiązania (ang. bindings) kodu Rust do C/C++. \cite{bindgen}
    \item Corrode eksperymentalny translator C do Rust.\cite{corrode}
\end{itemize}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\linewidth]{figures/c2rust-overview.png}
...
\begin{itemize}
    \item Firefox (Mozilla) - stopniowa migracja komponentów (np. silnik CSS Stylo), z wykorzystaniem Rust do poprawy bezpieczeństwa pamięci. Mozilla opracowała też RLBox, narzędzie do sandboxowania niebezpiecznego kodu C++.\cite{quantumstylo}

    \item Servo - eksperymentalna przeglądarka napisana całkowicie w Rust, której fragmenty (np. WebRender) zostały zintegrowane z Firefoxem.\cite{servo}

    \item Deno - (JavaScript lub TypeScript runtime) – używa Rust dla wydajnych modułów, podczas gdy core jest w C++.\cite{deno}

    \item Linux Kernel - od wersji 6.1 wspiera Rust jako drugi język systemowy, co umożliwia migrację wybranych modułów.\cite{linuxkernel}
...
\caption{Architektura Chrome w porównaniu do Firefox (na podstawie Firefox 57 "Quantum")}
...
Deno to nowoczesne środowisko wykonawcze dla JavaScript, TypeScript i WebAssembly\cite{deno}. Deno zostało napisane w Rust, co zapewnia mu wysoką wydajność i bezpieczeństwo. Głównym celem projektu było rozwiązanie problemów Node.js, takich jak złożony system zarządzania zależnościami i brak wsparcia dla TypeScript out-of-the-box.
